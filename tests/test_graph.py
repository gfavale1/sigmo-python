...
def assert_valid_csr_graph(graph):
    """
    Assert that a dictionary follows the SIGMo CSR graph schema.
    """
...
def test_smarts_to_csr_from_string_aromatic_is_stable():
    """
    Regression test: aromatic bonds must not use unsupported backend labels.

    The current backend-safe policy maps RDKit aromatic bond order 1.5 to
    integer label 1. This avoids passing unsupported explicit labels, such as
    12, to the native C++/SYCL backend.
    """
...
def test_invalid_smarts_are_skipped_without_crash(invalid_smarts_file):
    ...
    assert len(graphs) == 2
    assert all("NOT_A_MOLECULE" not in graph.get("name", "") for graph in graphs)
...
def test_load_molecules_from_list():
    ...
    assert all("input" in graph for graph in graphs)